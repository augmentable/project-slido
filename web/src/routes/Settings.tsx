...
type SessionFeatures = { id: string; title: string; code: string } & Record<keyof typeof FEATURE_DEFAULTS, boolean | null>;

export default function SettingsPage() {
  const { code } = useParams<{ code: string }>();
  const { data: sessionData } = useQuery(GET_SESSION, { variables: { code: code!.toUpperCase() } });
  const session = (sessionData as { session?: SessionFeatures })?.session;
  ...
  const adminSettings = (adminData as { adminSettings?: { openrouterKeySet: boolean; openrouterKeyPreview: string | null } })?.adminSettings;
  ...
  const handleAdminLogin = async () => {
    ...
    try {
      ...
      const token = (result.data as { adminLogin?: { token?: string } })?.adminLogin?.token;
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pb-4 animate-fade-in" style={{ borderBottom: '1px solid var(--border)' }}>
          <div>
            <div className="flex items-center gap-3 text-xs font-medium uppercase tracking-wider">
              <Link to={`/session/${code}`} className="hover:underline" style={{ color: 'var(--accent)' }}>&larr; Back to Session</Link>
            </div>
