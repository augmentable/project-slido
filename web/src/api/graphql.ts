...
const schema = createSchema({
  ...
  resolvers: {
    ...
    Mutation: {
      ...
      adminLogin: async (_: unknown, { password }: { password: string }, { db, adminPassword }: Ctx) => {
...
type Ctx = { db: Db; adminPassword: string | undefined };
...
// adminPassword comes from the Worker env rather than process.env: this
// compatibility_date leaves process.env empty even under nodejs_compat.
export function createGraphQLHandler(db: D1Database, adminPassword?: string) {
  const drizzleDb = getDb(db);
  return createYoga({
    schema,
    graphqlEndpoint: '/api/graphql',
    fetchAPI: { Response },
    context: () => ({ db: drizzleDb, adminPassword }),
  });
}
