...
export async function handleTitle(request: Request, d1: D1Database, fallbackKey?: string): Promise<Response> {
  let body: unknown;
  try {
  ...
  if (!apiKey) apiKey = fallbackKey?.trim() ?? '';
  if (!apiKey) {
    return Response.json(
  ...
  try {
    ...
    return Response.json({ error: 'Unable to generate a question title right now' }, { status: 503 });
  }
